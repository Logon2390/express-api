...
class Application {
  ...
  constructor() {
    ...
    this.chromaService = new ChromaService(config.chromaApi, logger);
    this.discordService = new DiscordService(config.discord, this.chromaService, logger);
    this.expressServer = new ExpressServer(config.server, this.chromaService, logger);
  }
  ...
  public async start(): Promise<void> {
    try {
      ...
      logger.info('│                                         │');
      logger.info(`│  Express API: http://localhost:${config.server.port.toString().padEnd(4)} │`);
